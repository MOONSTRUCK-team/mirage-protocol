...
const EXTERNAL_BRIDGE_URL: &str = "https://example.com/api/send-nft"; // Replace with actual external bridge API
...
//  Function to mint an NFT on another chain using HTTP outcall
#[update]
pub async fn mint_nft_on_external_chain(
    token_id: Nat,
    account: Account,
    ...
    let payload = serde_json::json!({
        ...
        "account_owner": account.owner.to_text(),
        "account_subaccount": account.subaccount.map(|sub| hex::encode(sub)),
        "metadata": metadata.iter().map(|entry| {
            serde_json::json!({
                "key": entry.key,
                "value": entry.value,
            })
    ...
    let result = http_request(request, 1_000_000).await;

    match result {
        Ok((response,)) => {
            if response.status == 200u64 {
                ic_cdk::print(format!(
                    "NFT mint successful on external chain: {:?}",
                    response.body
                ));
                Ok(())
            } else {
                Err(format!(
                    "Failed to mint NFT on external chain: HTTP {}",
                    response.status
                ))
            }
        }
